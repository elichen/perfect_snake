"""Train a neural policy on synthetic Hamiltonian states.

This is a feasibility probe for a pure neural Hamiltonian follower: every
training sample is an on-cycle Snake state, and the target is the next action
along that cycle. No planner or expert is used at inference. By default this
preserves the original cycle-conditioned control behavior; pass
``--no-cycle-conditioning`` to train a non-privileged student.
"""

from __future__ import annotations

import argparse
import json
import os
import time
from pathlib import Path
from typing import Any

import numpy as np
import torch
import torch.nn.functional as F

from distill.conditioning import augment_observation, conditioning_channels
from distill.evaluate import evaluate_policy
from distill.expert import expert_action, find_aligned_cycle
from distill.model import SnakePolicy, load_policy_state
from snake_env import SnakeEnv


def _save_atomic(payload: Any, path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    tmp = path.with_suffix(path.suffix + ".tmp")
    torch.save(payload, tmp)
    os.replace(tmp, path)


def _direction_from_body(head: tuple[int, int], neck: tuple[int, int]) -> int:
    dr = head[0] - neck[0]
    dc = head[1] - neck[1]
    for direction, delta in SnakeEnv.DIRECTIONS.items():
        if delta == (dr, dc):
            return int(direction)
    raise RuntimeError(f"non-adjacent head/neck: {head=} {neck=}")


class CycleStateSampler:
    def __init__(
        self,
        *,
        board_size: int,
        flood_fill: bool,
        head_centered: bool,
        seed: int,
        min_fill: float,
        max_fill: float,
        device: str,
        cycle_conditioning: bool,
        cycle_mode: str,
        cycle_index: int,
        cycle_indices: str | None,
        label_source: str,
    ) -> None:
        self.env = SnakeEnv(
            n=board_size,
            gamma=0.999,
            alpha=0.2,
            seed=seed,
            flood_fill_obs=flood_fill,
            head_centered=head_centered,
        )
        self.env.reset(seed=seed)
        self.board_size = board_size
        self.board_area = board_size * board_size
        self.min_len = max(3, int(np.floor(min_fill * self.board_area)))
        self.max_len = min(self.board_area - 1, int(np.ceil(max_fill * self.board_area)))
        self.rng = np.random.default_rng(seed)
        self.device = device
        self.cycle_conditioning = cycle_conditioning
        self.cycle_mode = cycle_mode
        self.cycle_index = cycle_index
        if cycle_indices:
            self.cycle_pool = tuple(
                int(part.strip())
                for part in cycle_indices.split(",")
                if part.strip()
            )
        else:
            self.cycle_pool = tuple(range(len(self.env._curriculum_cycles)))
        if not self.cycle_pool:
            raise ValueError("cycle_indices produced an empty cycle pool")
        for idx in self.cycle_pool:
            if not (0 <= idx < len(self.env._curriculum_cycles)):
                raise ValueError(f"cycle index {idx} must be in [0, {len(self.env._curriculum_cycles) - 1}]")
        self.label_source = label_source
        if self.cycle_mode == "fixed" and not (0 <= self.cycle_index < len(self.env._curriculum_cycles)):
            raise ValueError(f"cycle_index must be in [0, {len(self.env._curriculum_cycles) - 1}]")
        extra_channels = conditioning_channels(self.env) if cycle_conditioning else 0
        self.obs_shape = (
            self.env.observation_space.shape[0] + extra_channels,
            *self.env.observation_space.shape[1:],
        )

    def _sample_cycle_idx(self) -> int:
        if self.cycle_mode == "fixed":
            return int(self.cycle_index)
        return int(self.cycle_pool[int(self.rng.integers(len(self.cycle_pool)))])

    def sample_batch(self, batch_size: int) -> tuple[torch.Tensor, torch.Tensor]:
        obs_batch = np.empty((batch_size, *self.obs_shape), dtype=np.float32)
        act_batch = np.empty((batch_size,), dtype=np.int64)

        for idx in range(batch_size):
            cycle_idx = self._sample_cycle_idx()
            cycle = self.env._curriculum_cycles[cycle_idx]
            head_idx = int(self.rng.integers(len(cycle)))
            snake_len = int(self.rng.integers(self.min_len, self.max_len + 1))
            snake = [cycle[(head_idx + offset) % len(cycle)] for offset in range(snake_len)]

            occupied = set(snake)
            empty = [cell for cell in self.env._curriculum_cycles[0] if cell not in occupied]
            food = empty[int(self.rng.integers(len(empty)))] if empty else (-1, -1)

            self.env.snake = snake
            self.env.direction = _direction_from_body(snake[0], snake[1])
            self.env.food_pos = food
            self.env.score = snake_len - 3
            self.env.steps_since_food = 0
            self.env.total_steps = 0
            self.env._curriculum_cycle = None
            self.env._curriculum_head_idx = None
            self.env._obs_history_frames.clear()
            self.env._action_history.clear()
            self.env.prev_phi = self.env._compute_phi()

            if self.label_source == "first-aligned":
                label_cycle, label_head_idx = find_aligned_cycle(self.env)
                action, _ = expert_action(self.env, label_cycle, label_head_idx)
            else:
                action, _ = expert_action(self.env, cycle, head_idx)
            obs = self.env._get_observation()
            if self.cycle_conditioning:
                obs = augment_observation(obs, self.env, cycle_idx)
            obs_batch[idx] = obs
            act_batch[idx] = action

        return (
            torch.as_tensor(obs_batch, dtype=torch.float32, device=self.device),
            torch.as_tensor(act_batch, dtype=torch.long, device=self.device),
        )


@torch.no_grad()
def estimate_accuracy(
    *,
    policy: SnakePolicy,
    sampler: CycleStateSampler,
    batches: int,
    batch_size: int,
) -> dict[str, float]:
    correct = 0
    total = 0
    losses: list[float] = []
    for _ in range(batches):
        obs, target = sampler.sample_batch(batch_size)
        logits, _ = policy(obs)
        losses.append(float(F.cross_entropy(logits, target).item()))
        pred = torch.argmax(logits, dim=-1)
        correct += int((pred == target).sum().item())
        total += int(target.numel())
    return {
        "accuracy": float(correct / max(1, total)),
        "loss": float(np.mean(losses)) if losses else 0.0,
        "samples": float(total),
    }


def main() -> int:
    parser = argparse.ArgumentParser(description="Exhaustive synthetic Hamiltonian cycle BC")
    parser.add_argument("--board-size", type=int, default=20)
    parser.add_argument("--steps", type=int, default=5000)
    parser.add_argument("--batch-size", type=int, default=1024)
    parser.add_argument("--eval-batches", type=int, default=8)
    parser.add_argument("--eval-batch-size", type=int, default=1024)
    parser.add_argument("--eval-episodes", type=int, default=0)
    parser.add_argument("--eval-seed", type=int, default=10001)
    parser.add_argument("--lr", type=float, default=1e-4)
    parser.add_argument("--device", choices=["cpu", "cuda", "mps"], default="cpu")
    parser.add_argument("--network-scale", type=int, choices=[1, 2, 4], default=2)
    parser.add_argument("--flood-fill", action="store_true")
    parser.add_argument("--head-centered", action="store_true")
    parser.add_argument("--no-cycle-conditioning", action="store_false", dest="cycle_conditioning")
    parser.add_argument("--cycle-conditioning", action="store_true", dest="cycle_conditioning")
    parser.set_defaults(cycle_conditioning=True)
    parser.add_argument("--cycle-mode", choices=["random", "fixed"], default="random")
    parser.add_argument("--cycle-index", type=int, default=0)
    parser.add_argument("--cycle-indices", default=None)
    parser.add_argument("--label-source", choices=["sampled", "first-aligned"], default="sampled")
    parser.add_argument("--min-fill", type=float, default=0.0)
    parser.add_argument("--max-fill", type=float, default=1.0)
    parser.add_argument("--seed", type=int, default=44)
    parser.add_argument("--resume", type=Path, default=None)
    parser.add_argument("--save-path", type=Path, required=True)
    parser.add_argument("--log-every", type=int, default=100)
    args = parser.parse_args()

    torch.manual_seed(args.seed)
    np.random.seed(args.seed)

    train_sampler = CycleStateSampler(
        board_size=args.board_size,
        flood_fill=args.flood_fill,
        head_centered=args.head_centered,
        seed=args.seed,
        min_fill=args.min_fill,
        max_fill=args.max_fill,
        device=args.device,
        cycle_conditioning=args.cycle_conditioning,
        cycle_mode=args.cycle_mode,
        cycle_index=args.cycle_index,
        cycle_indices=args.cycle_indices,
        label_source=args.label_source,
    )
    eval_sampler = CycleStateSampler(
        board_size=args.board_size,
        flood_fill=args.flood_fill,
        head_centered=args.head_centered,
        seed=args.seed + 1_000_000,
        min_fill=args.min_fill,
        max_fill=args.max_fill,
        device=args.device,
        cycle_conditioning=args.cycle_conditioning,
        cycle_mode=args.cycle_mode,
        cycle_index=args.cycle_index,
        cycle_indices=args.cycle_indices,
        label_source=args.label_source,
    )

    policy = SnakePolicy(
        board_size=args.board_size,
        scale=args.network_scale,
        n_channels=train_sampler.obs_shape[0],
        aux_flood_fill=False,
        head_centered=args.head_centered,
    ).to(args.device)
    if args.resume is not None:
        state = torch.load(args.resume, map_location="cpu")
        load_policy_state(policy, state, aux_flood_fill=False, late_head_min_fill=None)

    optimizer = torch.optim.Adam(policy.parameters(), lr=args.lr)
    best_accuracy = -1.0
    start = time.time()
    args.save_path.parent.mkdir(parents=True, exist_ok=True)
    run_path = args.save_path.parent / "run.json"
    run_path.write_text(json.dumps({"args": vars(args), "obs_shape": train_sampler.obs_shape}, default=str, indent=2) + "\n")

    for step in range(1, args.steps + 1):
        policy.train()
        obs, target = train_sampler.sample_batch(args.batch_size)
        logits, _ = policy(obs)
        loss = F.cross_entropy(logits, target)

        optimizer.zero_grad(set_to_none=True)
        loss.backward()
        optimizer.step()

        if step == 1 or step % args.log_every == 0 or step == args.steps:
            policy.eval()
            acc = estimate_accuracy(
                policy=policy,
                sampler=eval_sampler,
                batches=args.eval_batches,
                batch_size=args.eval_batch_size,
            )
            event = {
                "step": step,
                "train_loss": round(float(loss.item()), 6),
                "eval_loss": round(acc["loss"], 6),
                "eval_accuracy": round(acc["accuracy"], 8),
                "eval_samples": int(acc["samples"]),
                "elapsed_sec": round(time.time() - start, 1),
            }
            if args.eval_episodes > 0:
                stats = evaluate_policy(
                    policy,
                    board_size=args.board_size,
                    episodes=args.eval_episodes,
                    seed=args.eval_seed,
                    deterministic=True,
                    flood_fill=args.flood_fill,
                    head_centered=args.head_centered,
                    device=args.device,
                    cycle_conditioning=args.cycle_conditioning,
                )
                event.update(
                    {
                        "episode_mean": round(float(stats["mean_score"]), 3),
                        "episode_win_rate": round(float(stats["win_rate"]), 6),
                        "episode_max": int(stats["max_score"]),
                    }
                )
            print(event, flush=True)

            if acc["accuracy"] > best_accuracy:
                best_accuracy = acc["accuracy"]
                _save_atomic(policy.state_dict(), args.save_path)
                best_path = args.save_path.with_name(f"{args.save_path.stem}.best_eval.pt")
                _save_atomic(policy.state_dict(), best_path)
                print(
                    {
                        "best_checkpoint": str(best_path),
                        "eval_accuracy": round(best_accuracy, 8),
                    },
                    flush=True,
                )

    _save_atomic(policy.state_dict(), args.save_path)
    print({"saved": str(args.save_path), "best_accuracy": round(best_accuracy, 8)}, flush=True)
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
